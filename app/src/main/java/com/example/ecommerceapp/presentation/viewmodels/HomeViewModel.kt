...
class HomeViewModel @Inject constructor(
    ...
    private var _categories = MutableLiveData<UiState<List<CategoryModel>>>()
    val categories: LiveData<UiState<List<CategoryModel>>> get() = _categories
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                _categories.value = when {
                    categoriesResult.isSuccess -> UiState.Success(
            ...
            favoriteIdsFlow.collect { favoriteIdsResult ->
                withContext(Dispatchers.Main) {
                    if (productsResult.isSuccess && favoriteIdsResult.isSuccess) {
                        ...
                        _products.value = UiState.Success(markedProducts)
           ...
           var result =   addProductToFavoritesByIdUseCase(id)
            withContext(Dispatchers.Main){
                if(result.isSuccess){
                    ...
                    _isProductAddedToFav.value = UiState.Error(R.string.process_failed_to_added_to_favorites)
                }
            }
        }
    }
        ...
        viewModelScope.launch (Dispatchers.IO){
            ...
            withContext(Dispatchers.Main){
                if(result.isSuccess){
                    ...
                    _isProductRemovedFromFav.value = UiState.Error(R.string.process_failed_to_remove_from_favorites)
                }
            }
        }
    }
